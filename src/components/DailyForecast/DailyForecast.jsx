...
//   function dayOfWeekAsString( getTime) {
//     const Time = new Date( getTime).getDay();
//     return ["Sunday", "Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"][Time] || '';
//   }
    ...
    <div className="daily-item-container">
      <h2>Daily Forecast</h2>
        { forecastDay.map((day, index) => (
                <>
                    <div className="daily-item" key={index} onClick={() => setOpenDailyDetails(!openDailyDetails)}>
                        <div className="text-1">
                            <img className="icon" src={day?.day?.condition?.icon} alt="icons" />
                            <label className="day">{ day?.date }</label>
                        </div>
